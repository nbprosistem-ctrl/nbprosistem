...
import { CheckCircle, Clock, MoreVertical, Shield, UserX, Trash2, UserCheck } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await axios.get(`${import.meta.env.VITE_API_URL || "https://nbprosistem-backend.onrender.com"}/api/admin/users`, {
  ...
  const handleApprove = async (e, id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.patch(`${import.meta.env.VITE_API_URL || "https://nbprosistem-backend.onrender.com"}/api/admin/users/${id}/approve`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleToggleBlock = async (e, id, currentBlocked) => {
    e.stopPropagation();
    if (!window.confirm(`Tem certeza que deseja ${currentBlocked ? 'desbloquear' : 'bloquear'} este usuário?`)) return;
    try {
      const token = localStorage.getItem('token');
      const response = await axios.patch(`${import.meta.env.VITE_API_URL || "https://nbprosistem-backend.onrender.com"}/api/admin/users/${id}/block`, 
  ...
  const handleToggleRole = async (e, id, currentRole) => {
    e.stopPropagation();
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.patch(`${import.meta.env.VITE_API_URL || "https://nbprosistem-backend.onrender.com"}/api/admin/users/${id}/role`, 
        { role: newRole }, 
        { headers: { Authorization: `Bearer ${token}` }}
      );
      setUsers(users.map(u => u.id === id ? response.data.user : u));
  ...
  const handleDeleteUser = async (e, id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${import.meta.env.VITE_API_URL || "https://nbprosistem-backend.onrender.com"}/api/admin/users/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      <div className="app-main">
        ...
        <main className="main-content">
          ...
          {loading ? (
            ...
            <div style={{ overflowX: 'auto', paddingBottom: '180px', marginBottom: '-180px' }}>
                            ...
                            {openMenuId === u.id && (
                              <div 
                                ...
                                style={{
                                  ...
                                  ...(index >= users.length - 2 ? { bottom: 0 } : { top: 0 }),
                                  zIndex: 100,
                              ...
                              >
                                {u.status === 'PENDING' && (
                                  <button onClick={(e) => handleApprove(e, u.id)} className="dropdown-item">
                                    <UserCheck size={14} /> Aprovar agora
                                  </button>
                                )}
                                 
                                <button onClick={(e) => handleToggleRole(e, u.id, u.role)} className="dropdown-item">
                                  <Shield size={14} /> {u.role === 'ADMIN' ? 'Rebaixar p/ Colaborador' : 'Promover para Admin'}
                                </button>
   
                                <button 
                                  onClick={(e) => handleToggleBlock(e, u.id, u.is_blocked)} 
