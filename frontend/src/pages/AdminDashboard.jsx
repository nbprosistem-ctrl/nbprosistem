...
export default function AdminDashboard() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await axios.get(`${import.meta.env.VITE_API_URL || "https://nbprosistem.onrender.com"}/api/admin/users`, {
  ...
  const handleApprove = async (e, id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.patch(`${import.meta.env.VITE_API_URL || "https://nbprosistem.onrender.com"}/api/admin/users/${id}/approve`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleToggleBlock = async (e, id, currentBlocked) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.patch(`${import.meta.env.VITE_API_URL || "https://nbprosistem.onrender.com"}/api/admin/users/${id}/block`, 
  ...
  const handleToggleRole = async (e, id, currentRole) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.patch(`${import.meta.env.VITE_API_URL || "https://nbprosistem.onrender.com"}/api/admin/users/${id}/role`, 
        { role: newRole }, 
        { headers: { Authorization: `Bearer ${token}` }}
      );
      setUsers(users.map(u => u.id === id ? response.data.user : u));
  ...
  const handleDeleteUser = async (e, id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${import.meta.env.VITE_API_URL || "https://nbprosistem.onrender.com"}/api/admin/users/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
