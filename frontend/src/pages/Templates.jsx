...
export default function Templates() {
  ...
  const fetchTemplates = async () => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_API_URL || "http://localhost:3001"}/api/templates`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${import.meta.env.VITE_API_URL || "http://localhost:3001"}/api/templates`, {
  ...
  const handleDelete = async (id, tmplName) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${import.meta.env.VITE_API_URL || "http://localhost:3001"}/api/templates/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchTemplates();
    } catch (err) {
