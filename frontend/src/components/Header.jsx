...
export default function Header({ title }) {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_API_URL || "https://nbprosistem-backend.onrender.com"}/api/notifications`, {
  ...
  const markAsRead = async (id) => {
    try {
      const token = localStorage.getItem('token');
      await axios.patch(`${import.meta.env.VITE_API_URL || "https://nbprosistem-backend.onrender.com"}/api/notifications/${id}/read`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchNotifications();
    } catch (err) {
  ...
  const markAllAsRead = async () => {
    try {
      ...
      await Promise.all(
         unreadsIds.map(id => axios.patch(`${import.meta.env.VITE_API_URL || "https://nbprosistem-backend.onrender.com"}/api/notifications/${id}/read`, {}, {
           headers: { Authorization: `Bearer ${token}` }
         }))
      );
      fetchNotifications();
