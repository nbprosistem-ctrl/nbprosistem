...
export default function VaultModal({ entry, onClose, onSave, onToast }) {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_API_URL || "https://nbprosistem-backend.onrender.com"}/api/admin/users`, {
  ...
  const fetchEntryAccess = async (id) => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get(`${import.meta.env.VITE_API_URL || "https://nbprosistem-backend.onrender.com"}/api/vault/${id}/access`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSaving(true);
      const token = localStorage.getItem('token');
      const url = `${import.meta.env.VITE_API_URL || "https://nbprosistem-backend.onrender.com"}/api/vault`;
